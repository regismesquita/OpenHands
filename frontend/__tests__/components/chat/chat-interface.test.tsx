...
describe("Empty state", () => {
  ...
  const { useWebSocketClient: useWebSocketClientMock } = vi.hoisted(() => ({
    useWebSocketClient: vi.fn(() => ({ send: sendMock, runtimeActive: true })),
  }));

  beforeAll(() => {
    vi.mock("#/context/web-socket-client", async (importActual) => ({
      ...(await importActual<typeof import("#/context/web-socket-client")>()),
      useWebSocketClient: useWebSocketClientMock,
    }));
  });
  ...
  it.fails(
    ...
    async () => {
      ...
      useWebSocketClientMock.mockImplementation(() => ({
        send: sendMock,
